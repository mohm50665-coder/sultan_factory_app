...
export const production = mysqlTable("production", {
  ...
  movementAt: timestamp("movementAt"),
...
export const manufacturingStages = mysqlTable("manufacturingStages", {
  ...
  stageStartedAt: timestamp("stageStartedAt"),
  stageCompletedAt: timestamp("stageCompletedAt"),
  shortageDozen: int("shortageDozen").default(0),
  shortagePairs: int("shortagePairs").default(0),
  wasteGrams: int("wasteGrams").default(0),
  userId: int("userId").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export const productTracking = mysqlTable("productTracking", {
  ...
  qualityGrade: mysqlEnum("qualityGrade", ["first", "second", "sample"]),
  sampleRequestId: int("sampleRequestId"),
  sampleReference: varchar("sampleReference", { length: 30 }),
...
// طلبات العينات المرتبطة بسجل الإنتاج والعهدة دون إنشاء هوية منتج مكررة
export const sampleRequests = mysqlTable("sampleRequests", {
  id: int("id").autoincrement().primaryKey(),
  referenceCode: varchar("referenceCode", { length: 30 }).notNull().unique(),
  requesterId: int("requesterId").notNull(),
  requesterName: varchar("requesterName", { length: 255 }).notNull(),
  requesterDepartment: varchar("requesterDepartment", { length: 100 }),
  clientName: varchar("clientName", { length: 255 }),
  productName: varchar("productName", { length: 255 }).notNull(),
  productSize: varchar("productSize", { length: 100 }).notNull(),
  productColor: varchar("productColor", { length: 100 }).notNull(),
  quantity: int("quantity").notNull(),
  quantityUnit: mysqlEnum("quantityUnit", ["dozen", "pair"]).default("dozen").notNull(),
  specifications: text("specifications").notNull(),
  designAttachments: json("designAttachments").notNull(),
  manufacturingFormAttachments: json("manufacturingFormAttachments").notNull(),
  transferReceiptAttachments: json("transferReceiptAttachments").notNull(),
  signatureAttachments: json("signatureAttachments").notNull(),
  sampleMediaAttachments: json("sampleMediaAttachments"),
  status: mysqlEnum("status", ["incomplete", "pending_sales", "approved_sales", "rejected_sales", "approved_production", "in_production", "ready_for_requester", "completed"]).default("incomplete").notNull(),
  productionId: int("productionId"),
