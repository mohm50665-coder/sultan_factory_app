...
export default function BarcodeLabelsScreen() {
  ...
  const buildPrintHtml = () => {
    ...
    const oneLabel = `<div class="label"><div class="name">${safeName}</div><div class="color">${isAr ? "اللون" : "Color"}: ${safeColor}</div><div class="bars">${barsHtml}</div><div class="code">${safeBarcode}</div><div class="footer">مصنع السلطان للجوارب<br/><span>Sultan Socks Factory</span></div></div>`;
    return `<!doctype html><html dir="${isAr ? "rtl" : "ltr"}"><head><meta charset="utf-8"><style>@page{size:${labelWidth} ${labelHeight};margin:0}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif}.sheet{display:flex;flex-wrap:wrap;gap:2mm;padding:1mm}.label{width:${labelWidth};height:${labelHeight};padding:2mm;text-align:center;border:0.2mm solid #222;overflow:hidden;break-inside:avoid}.name{font-weight:700;font-size:12px;white-space:nowrap;overflow:hidden}.color{font-size:9px}.bars{height:42px;display:flex;justify-content:center;overflow:hidden;margin:2px 0}.code{font-size:9px;letter-spacing:1px}.footer{font-size:7px;margin-top:2px;line-height:1.15}.footer span{font-size:6px}</style></head><body><div class="sheet">${Array.from({ length: labelCount }, () => oneLabel).join("")}</div></body></html>`;
  };
    ...
    <ScreenContainer containerClassName="bg-background" safeAreaClassName="bg-background">
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.twoCol}><View style={styles.col}><Text style={styles.label}>{isAr ? "عدد النسخ للطباعة" : "Copies to print"}</Text><TextInput value={quantity} onChangeText={setQuantity} keyboardType="numeric" style={styles.input} /></View><View style={styles.col}><Text style={styles.label}>{isAr ? "الوحدة" : "Unit"}</Text><TextInput value={unit} onChangeText={setUnit} style={styles.input} /></View></View>
        </View>
        <View style={styles.card}>
          ...
          <Text style={styles.selected}>{isAr ? `المحدد: ${selectedSize.widthMm} × ${selectedSize.heightMm} مم — عدد النسخ للطباعة: ${labelCount}` : `Selected: ${selectedSize.widthMm} × ${selectedSize.heightMm} mm — Copies: ${labelCount}`}</Text>
        </View>
        <View style={styles.card}>
