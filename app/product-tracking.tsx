...
export default function ProductTrackingScreen() {
  ...
  const printHandoverReport = () => {
    ...
    const rows = filteredHandovers.map((row) => { const catalog = catalogForProduct(String(row.productName || "")); const deliveredEmployee = employeeDetailsLabel(row.deliveredBy); const expectedEmployee = employeeDetailsLabel(row.expectedReceiver); const receivedEmployee = employeeDetailsLabel(row.receivedBy); const location = locationForProduct(String(row.productName || "")); const status = row.handoverStatus === "pending" ? "بانتظار الاستلام" : row.receivedBy ? "قيد التشغيل" : location.status; const receivedQty = `${numberValue(row.receivedQuantityDozen ?? row.quantityDozen)} درزن + ${numberValue(row.receivedQuantityPairs ?? row.quantityPairs)} زوج`; const deliveredQty = `${numberValue(row.deliveredQuantityDozen ?? row.quantityDozen)} درزن + ${numberValue(row.deliveredQuantityPairs ?? row.quantityPairs)} زوج`; const shortage = `${numberValue(row.shortageDozen)} درزن + ${numberValue(row.shortagePairs)} زوج`; return `<tr><td>${escapeHtml(row.productName || "-")}</td><td>${escapeHtml(row.productBarcode || catalog.barcode || "غير محدد")}</td><td>${escapeHtml(row.productColor || catalog.color || "غير محدد")}</td><td>${escapeHtml(row.productSize || catalog.size || "غير محدد")}</td><td>${escapeHtml(stageLabel(row.previousStage))} ← ${escapeHtml(stageLabel(row.currentStage))}</td><td>${escapeHtml(status)}</td><td>${escapeHtml(location.label)}</td><td>${escapeHtml(deliveredEmployee || "-")}</td><td>${escapeHtml(expectedEmployee || "غير محدد")}<br/>المرحلة: ${escapeHtml(stageLabel(row.receiverStage))}</td><td>${escapeHtml(receivedEmployee || "بانتظار الاستلام")}</td><td>${escapeHtml(formatActionTime(row.deliveredAt))}</td><td>${escapeHtml(formatActionTime(row.receivedAt))}</td><td>${escapeHtml(elapsedLabel(elapsedMinutes(row.deliveredAt, row.receivedAt), true))}</td><td>${escapeHtml(receivedQty)}</td><td>${escapeHtml(deliveredQty)}</td><td>${escapeHtml(shortage)}</td><td>${escapeHtml(row.notes || "-")}</td></tr>`; }).join("");
