...
const defaultSaturdayThursdayRange = () => {
  ...
  start.setDate(current.getDate() - ((current.getDay() + 1) % 7));
...
export default function ProductTrackingScreen() {
  ...
  const defaultRange = defaultSaturdayThursdayRange();
  const [dateFilter, setDateFilter] = useState(defaultRange.start);
  const [dateToFilter, setDateToFilter] = useState(defaultRange.end);
  const [appliedDateFilter, setAppliedDateFilter] = useState(defaultRange.start);
  const [appliedDateToFilter, setAppliedDateToFilter] = useState(defaultRange.end);
  ...
  const employeeCompactLabel = (name: unknown) => {
    const employee = employeeFor(name);
  ...
  const downloadHandoverFile = (format: "word" | "excel") => {
    ...
    const detailRows = filteredHandovers.map((row, index) => {
      ...
      return `<tr><td>${index + 1}</td><td>${escapeHtml(row.productName || "غير محدد")}؛ المقاس: ${escapeHtml(row.productSize || catalog.size || "غير محدد")}؛ اللون: ${escapeHtml(row.productColor || catalog.color || "غير محدد")}؛ الباركود: ${escapeHtml(row.productBarcode || catalog.barcode || "غير محدد")}</td><td>${escapeHtml(stageLabel(row.previousStage))} ← ${escapeHtml(stageLabel(row.currentStage))}؛ المرحلة التالية: ${escapeHtml(stageLabel(row.receiverStage))}؛ الموقع الحالي: ${escapeHtml(location.label)}</td><td>اليوم والتاريخ والوقت للتسليم: ${escapeHtml(formatActionTime(row.deliveredAt))}؛ اليوم والتاريخ والوقت للاستلام: ${escapeHtml(formatActionTime(row.receivedAt))}؛ <strong>مدة بقاء المنتج لدى المرحلة: ${escapeHtml(elapsedLabel(duration, true))}</strong></td><td>الموظف المسلّم: ${escapeHtml(employeeCompactLabel(row.deliveredBy) || "غير محدد")}؛ المستلم المحدد: ${escapeHtml(employeeCompactLabel(row.expectedReceiver) || "غير محدد")}؛ المستلم الفعلي: ${escapeHtml(employeeCompactLabel(row.receivedBy) || "لم يؤكد بعد")}</td><td>التسليم: ${escapeHtml(deliveredQty)}؛ الاستلام: ${escapeHtml(receivedQty)}؛ الفرق/النقص: ${escapeHtml(difference)}</td><td>${escapeHtml(status)}؛ ${escapeHtml(row.notes || "لا توجد ملاحظات")}</td></tr>`;
    }).join("");
  ...
  const printHandoverReport = () => {
    ...
    printWindow.document.write(`<!doctype html><html dir="rtl"><head><meta charset="utf-8"><title>تقرير تفصيلي للتسليم والاستلام</title><style>body{font-family:Arial,sans-serif;padding:22px;color:#17202a;line-height:1.55}h1,h2{text-align:right;color:#075985;margin:8px 0}.meta{background:#eff8fb;border:1px solid #b8dce8;padding:12px;border-radius:8px;text-align:right}.note{background:#fff7ed;border:1px solid #fed7aa;padding:10px;margin:12px 0;text-align:right}table{width:100%;border-collapse:collapse;margin:10px 0 24px;font-size:11px;page-break-inside:auto}th,td{border:1px solid #9fb5bd;padding:8px;text-align:right;vertical-align:top}th{background:#075985;color:white;font-size:12px}tr{page-break-inside:avoid}tr:nth-child(even){background:#f7fbfc}hr{border:0;border-top:1px solid #d7e2e6;margin:5px 0}.duration{display:block;background:#e0f2fe;color:#075985;padding:4px;margin-top:5px;border-radius:4px}.danger{color:#b91c1c}.ok{color:#15803d}@page{size:A4 landscape;margin:8mm}thead{display:table-header-group}tfoot{display:table-footer-group}td,th{font-size:9px;padding:3px;white-space:normal}.meta,.note{font-size:10px;padding:8px}h1{font-size:18px}h2{font-size:14px}.duration{padding:3px;margin-top:3px}@media print{button{display:none}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><h1>التقرير التفصيلي للتسليم والاستلام في مراحل الإنتاج</h1><div class="meta"><strong>نطاق التقرير:</strong> ${escapeHtml(appliedDateFilter || appliedDateToFilter ? `${appliedDateFilter || "بداية غير محددة"} إلى ${appliedDateToFilter || "نهاية غير محددة"}` : "السبت إلى الخميس الحالي")}؛ <strong>تاريخ ووقت طباعة التقرير:</strong> ${escapeHtml(formatActionTime(new Date()))}؛ <strong>المنتج:</strong> ${escapeHtml(productFilter || "جميع المنتجات")} | <strong>المرحلة:</strong> ${escapeHtml(stageFilter === "all" ? "جميع المراحل" : stageLabel(stageFilter))} | <strong>الموظف:</strong> ${escapeHtml(employeeFilter || "جميع الموظفين")}</div><div class="note"><strong>طريقة قراءة التقرير:</strong> كل صف يمثل حركة واحدة بين مرحلتين. يظهر اليوم والتاريخ والساعة للتسليم والاستلام بشكل منفصل، ثم مدة بقاء المنتج لدى المرحلة، مع بيان الموظف المسلم والمستلم المحدد والمستلم الفعلي، والفرق بين الكمية المسلّمة والكمية المستلمة.</div><h2>أولاً: ملخص مسؤولية الموظفين</h2><table><thead><tr><th>الموظف</th><th>عدد التسليم</th><th>كمية التسليم</th><th>عدد الاستلام</th><th>كمية الاستلام</th></tr></thead><tbody>${summaryRows || '<tr><td colspan="5">لا توجد بيانات</td></tr>'}</tbody></table><h2>ثانياً: سجل الحركات التفصيلي</h2><table><thead><tr><th>#</th><th>بيانات المنتج</th><th>المرحلة والمسار</th><th>اليوم والتاريخ والوقت والمدة</th><th>المسؤولية والموظفون</th><th>الكميات والفرق</th><th>الحالة والملاحظات</th></tr></thead><tbody>${detailRows || '<tr><td colspan="7">لا توجد بيانات ضمن الفلاتر الحالية</td></tr>'}</tbody></table><script>window.onload=()=>window.print()</script></body></html>`);
