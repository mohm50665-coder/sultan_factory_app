...
export default function MeetingRequestScreen() {
  ...
  const handleSaveMeeting = async () => {
    if (!form.title || !form.date) {
      Alert.alert(isAr ? "خطأ" : "Error", isAr ? "الرجاء إدخال عنوان الاجتماع والتاريخ" : "Please enter meeting title and date");
      return;
    }
    if (form.attendees.length === 0) {
      Alert.alert(isAr ? "خطأ" : "Error", isAr ? "الرجاء تحديد الأعضاء المعنيين بالاجتماع" : "Please select meeting attendees");
      return;
    }
    ...
    try {
      ...
      await loadData();
      setShowForm(false);
      Alert.alert(isAr ? "نجح" : "Success", editingMeeting ? isAr ? "تم تحديث الاجتماع" : "Meeting updated" : isAr ? "تم جدولة الاجتماع بنجاح" : "Meeting scheduled successfully");
    } catch (error) {
      Alert.alert(isAr ? "خطأ" : "Error", isAr ? "فشل في حفظ الاجتماع" : "Failed to save meeting");
    }
  };

  const handleDeleteMeeting = (meeting: Meeting) => {
    Alert.alert(isAr ? "تأكيد الحذف" : "Confirm Deletion", isAr ? `هل تريد حذف الاجتماع رقم #${meeting.meetingNumber}؟` : `Do you want to delete meeting #${meeting.meetingNumber}?`, [
      { text: isAr ? "إلغاء" : "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert(isAr ? "خطأ" : "Error", isAr ? "فشل في حذف الاجتماع" : "Failed to delete meeting");
          }
        },
      },
    ]);
  ...
  const handleOpenLink = (link: string) => {
    if (link) {
      ...
      Linking.openURL(url).catch(() => Alert.alert(isAr ? "خطأ" : "Error", isAr ? "لا يمكن فتح الرابط" : "Cannot open link"));
    }
  };
  ...
  return (
    <ScreenContainer>
      ...
      <View style={[styles.header, { backgroundColor: "#1E3A5F" }]}>
        <BackButton />
        <Text style={styles.headerTitle}>{isAr ? "طلب اجتماع" : "Meeting Request"}</Text>
      ...
      <FlatList
            ...
            <View style={{ gap: 4, marginBottom: 8 }}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                ...
                <Text style={{ color: colors.muted, fontSize: 12 }}>
                  {item.attendees.length} {isAr ? "عضو:" : "Attendees:"} {item.attendees.map(a => getUserName(a)).join(isAr ? "، " : ", ")}
                </Text>
              </View>
              {item.attachments.length > 0 && (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                  <MaterialIcons name="attach-file" size={14} color={colors.muted} />
                  <Text style={{ color: colors.muted, fontSize: 12 }}>{item.attachments.length} {isAr ? "مرفق" : "Attachment(s)"}</Text>
                </View>
              )}
            </View>
            ...
            <View style={{ flexDirection: "row", gap: 6, marginTop: 8, borderTopWidth: 0.5, borderColor: colors.border, paddingTop: 8 }}>
              {item.status === "scheduled" && (
                ...
                >
                  ...
                  <Text style={{ color: "#10B981", fontSize: 11 }}>{isAr ? "اكتمل" : "Completed"}</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
        ...
        ListEmptyComponent={
          <View style={{ alignItems: "center", paddingVertical: 60 }}>
            ...
            <Text style={{ color: colors.muted, marginTop: 12, fontSize: 15 }}>{isAr ? "لا توجد اجتماعات" : "No meetings"}</Text>
        ...
        <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
          <View style={[styles.modalHeader, { borderColor: colors.border }]}>
            ...
            <Text style={{ color: colors.foreground, fontWeight: "bold", fontSize: 16 }}>
              {editingMeeting ? (isAr ? `تعديل اجتماع #${editingMeeting.meetingNumber}` : `Edit Meeting #${editingMeeting.meetingNumber}`) : (isAr ? "طلب اجتماع جديد" : "New Meeting Request")}
            </Text>
          ...
          <ScrollView contentContainerStyle={{ padding: 16, gap: 14 }}>
            ...
            {/* Attendees */}
            <View>
              <Text style={[styles.label, { color: colors.foreground }]}>{isAr ? `الأعضاء المعنيون بالاجتماع * (${form.attendees.length} عضو)` : `Meeting Attendees * (${form.attendees.length} attendees)`}</Text>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={{ color: colors.primary, fontSize: 13, fontWeight: "600" }}>{isAr ? "اختيار الأعضاء" : "Select Attendees"}</Text>
              </TouchableOpacity>
            ...
            {/* Attachments */}
            <View>
              <Text style={[styles.label, { color: colors.foreground }]}>{isAr ? "المرفقات والنماذج" : "Attachments and Forms"}</Text>
              <TouchableOpacity
                style={[styles.selectBtn, { backgroundColor: colors.surface, borderColor: colors.border }]}
        ...
        <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
          <View style={[styles.modalHeader, { borderColor: colors.border }]}>
            <TouchableOpacity onPress={() => setShowAttendeesPicker(false)}>
              <Text style={{ color: colors.primary, fontWeight: "600" }}>{isAr ? "تم" : "Done"}</Text>
            </TouchableOpacity>
            <Text style={{ color: colors.foreground, fontWeight: "bold", fontSize: 16 }}>{isAr ? "اختيار الأعضاء" : "Select Attendees"}</Text>
