...
export default function ProductsScreen() {
  ...
  const save = async () => {
    ...
    if (!name.trim() || !size.trim() || !color.trim() || !weight.trim() || Number(weight) <= 0 || !yarnWeightPerPair.trim() || Number(yarnWeightPerPair) <= 0 || Object.values(yarnWeights).every((value) => value <= 0)) return Alert.alert(isAr ? "بيانات إلزامية" : "Required data", isAr ? "يجب إدخال الاسم والمقاس واللون ووزن الجورب ووزن الخيط/زوج ووزن نوع خيط واحد على الأقل قبل الحفظ" : "Enter the name, size, color, sock weight, thread weight per pair, and at least one thread type before saving");
  ...
  const sortedItems = useMemo(() => {
    ...
    const filtered = items.filter((product) => {
      ...
      return matchesQuery && (from === null || (!Number.isNaN(timestamp) && timestamp >= from)) && (to === null || (!Number.isNaN(timestamp) && timestamp <= to));
    });
  ...
  }, [items, searchQuery, dateFrom, dateTo, sortMode]);
  ...
  return <ScreenContainer className="p-4"><ScrollView contentContainerStyle={styles.container}>
    ...
    <View style={styles.card}><Text style={styles.section}>{editing ? (isAr ? "تعديل بيانات المنتج" : "Edit product details") : (isAr ? "إضافة منتج جديد" : "Add new product")}</Text><Text style={styles.label}>{isAr ? "اسم المنتج *" : "Product name *"}</Text><TextInput value={name} onChangeText={setName} style={styles.input} placeholder={isAr ? "مثال: ECO" : "Example: ECO"} />
      ...
      <View style={styles.row}><Pressable onPress={save} style={styles.primary}><MaterialIcons name="save" size={19} color="#fff" /><Text style={styles.primaryText}>{editing ? (isAr ? "حفظ التعديل" : "Save changes") : (isAr ? "حفظ وإنشاء باركود" : "Save and create barcode")}</Text></Pressable>{editing ? <Pressable onPress={reset} style={styles.secondary}><Text>{isAr ? "إلغاء" : "Cancel"}</Text></Pressable> : null}</View>
    </View>
    <View style={styles.card}><View style={styles.listHeader}><Text style={styles.section}>{isAr ? "بيانات المنتجات" : "Products"} ({sortedItems.length})</Text><View style={styles.row}><Pressable onPress={printAllProducts} style={styles.reportButton}><MaterialIcons name="print" size={18} color="#0a7ea4" /><Text style={styles.reportText}>{isAr ? "طباعة التقرير" : "Print report"}</Text></Pressable><Pressable onPress={load}><MaterialIcons name="refresh" size={22} color="#0a7ea4" /></Pressable></View></View>
      <TextInput value={searchQuery} onChangeText={setSearchQuery} style={styles.input} placeholder={isAr ? "بحث بالاسم أو الباركود أو المقاس أو اللون" : "Search by name, barcode, size, or color"} placeholderTextColor="#8a98a2" />
      <View style={[styles.row, { justifyContent: "flex-end", flexWrap: "wrap" }]}><TextInput value={dateFrom} onChangeText={setDateFrom} style={[styles.input, { flex: 1, minWidth: 150 }]} placeholder={isAr ? "من تاريخ YYYY-MM-DD" : "From date YYYY-MM-DD"} placeholderTextColor="#8a98a2" /><TextInput value={dateTo} onChangeText={setDateTo} style={[styles.input, { flex: 1, minWidth: 150 }]} placeholder={isAr ? "إلى تاريخ YYYY-MM-DD" : "To date YYYY-MM-DD"} placeholderTextColor="#8a98a2" /><Pressable onPress={() => { setDateFrom(""); setDateTo(""); }} style={styles.filterButton}><Text style={styles.filterText}>{isAr ? "مسح التاريخ" : "Clear dates"}</Text></Pressable></View>
      ...
      {loading ? <ActivityIndicator color="#0a7ea4" /> : sortedItems.length === 0 ? <Text style={styles.empty}>{isAr ? "لا توجد منتجات محفوظة بعد" : "No saved products yet"}</Text> : sortedItems.map((product) => { const detail = stats[product.id] || emptyStats(); const expanded = expandedId === product.id; return <View key={product.id} style={styles.itemWrap}>
